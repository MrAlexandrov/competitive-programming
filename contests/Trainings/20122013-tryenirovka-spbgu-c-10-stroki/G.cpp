...
public:
    ...
    static Modular CountReversedHash(const std::string& other);

    Modular GetHash(size_t l, size_t r) const;
    Modular GetReversedHash(size_t l, size_t r) const;
...
std::vector<Modular> THash::InversedPower_ = GeneratePowers(SmallStringOptimizationMaximum, NModular::Inverse(SmallNumber_, Modular::GetModule()));
...
Modular THash::CountReversedHash(const std::string& other) {
    EnsureCapacity(other.size());
    ...
    for (size_t i = 0; i < n; ++i) {
        result += Codes_[other[i] - 'a'] * Power_[i];
    }
    return result;
}
...
void THash::EnsureCapacity(size_t n) {
    if (Power_.size() < n) {
        ...
        Power_.reserve(n);
        InversedPower_.reserve(n);

        Modular base = SmallNumber_;
        Modular inverseBase = NModular::Inverse(SmallNumber_, Modular::GetModule());

        for (size_t i = oldSize; i < n; ++i) {
            Power_.emplace_back(Power_.back() * base);
            InversedPower_.emplace_back(InversedPower_.back() * inverseBase);
        }
    }
}
...
void THash::ComputeReversedHash() {
    ReversedHash_.resize(Data_.size());
    if (Data_.empty()) return;

    ReversedHash_[0] = Codes_[Data_[0] - 'a'];
    for (size_t i = 1; i < Data_.size(); ++i) {
        ReversedHash_[i] = ReversedHash_[i - 1] + Codes_[Data_[i] - 'a'] * Power_[i];
    }
}
...
Modular THash::GetReversedHash(size_t l, size_t r) const {
    if (!(l < r)) {
        #if defined (DEBUG)
        std::cout << "!(l < r)\n";
        #endif // DEBUG
    ...
    if (l == 0) {
        return ReversedHash_[r - 1];
    }
    return (ReversedHash_[r - 1] - ReversedHash_[l - 1]) * InversedPower_[l];
}
...
int main() {
    ...
    for (int i = 0; i < k; ++i) {
        ...
        auto currentHash = NHash::THash::CountHash(word);
        #if defined (DEBUG)
        std::cout << currentHash << std::endl;
        #endif // DEBUG
        bool wasAnswer = false;
        for (int j = 0; j + m - 1 < n && !wasAnswer; ++j) {
            #if defined (DEBUG)
            std::cout << hash.GetHash(j, j + m) << std::endl;
            #endif // DEBUG
            if (hash.GetHash(j, j + m) == currentHash) {
                std::cout << "Yes\n";
                wasAnswer = true;
            }
        }
